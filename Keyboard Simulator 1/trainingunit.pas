...
begin
  ...
  ShowMessage('Правильно: ' + CorrectDisplay.Text +
    '  Неправильно: ' + ErrorDisplay.Text + '  Срзнач: ' +
    AverageDisplay.Text);
end;
...
begin
  ...
  ErrorDisplay.Text := IntToHex(key, 4);
end;
...
begin
  ...
  begin
    ...
    if f then
      CorrectDisplay.Text := IntToStr(StrToIntDef(CorrectDisplay.Text, 0) + 1)
    else
      begin
        ErrorDisplay.Text := IntToStr(StrToIntDef(ErrorDisplay.Text, 0) + 1);
...
begin
  inputEdit.SetFocus;
  t := StrToIntDef(TimeDisplay.Text, 60) - 1;
  TimeDisplay.Text := IntToStr(t);
  if t < 1 then
    StopBtnClick(Sender);
  dt := StrToIntDef(TimeEdit.Text, 60) - t;
  sum := StrToIntDef(CorrectDisplay.Text, 0) - StrToIntDef(ErrorDisplay.Text, 0);
  if sum < 0 then
    sum := 0;
  if dt <> 0 then
    avg := Round(sum * 60 / dt)
    ...
    avg := 0;
  AverageDisplay.Text := IntToStr(avg);
end;
...
const
  ...
  // Задержка появления следующей буквы в долях суток
var
